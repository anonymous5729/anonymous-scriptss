...
Title.TextSize = 18
Title.Text = "Bandu’s Admin" .. currentVersion
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Bandu’s Admin Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs[#CMDs + 1] = {NAME = 'hkillall', DESC = 'nan'}
...
addcmd('airwalk', {}, function(args, speaker)
    local player = game.Players.LocalPlayer

    -- Encapsular tudo para evitar duplicação na mesma sessão
    if getgenv()._airwalkGui and getgenv()._airwalkGui.Parent then
        if args[1] and args[1]:lower() == "off" then
            if getgenv()._airwalkPlatform then pcall(function() getgenv()._airwalkPlatform:Destroy() end) end
            getgenv()._airwalkGui:Destroy()
            getgenv()._airwalkGui = nil
            getgenv()._airwalkPlatform = nil
        end
        return
    end
    ...
    getgenv()._airwalkPlatform = platform
...
addcmd("hkillall", {}, function(args, speaker)
    execCmd("hkillall")
    if not firetouchinterest then
